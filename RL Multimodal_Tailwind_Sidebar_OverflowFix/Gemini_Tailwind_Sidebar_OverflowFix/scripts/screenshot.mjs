  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      headerWithinViewport:
        hb && hb.top >= 0 && hb.bottom <= window.innerHeight,
      footerWithinViewport:
        fb && fb.top >= 0 && fb.bottom <= window.innerHeight,
      footerBottomIsAtViewportBottom:
        fb ? Math.round(window.innerHeight - fb.bottom) : null,
      navScrolls: nav ? nav.scrollHeight > nav.clientHeight : null,
      navScrollableClassApplied: nav
...
try {
  ...
  // Before, desktop
  results.push({ shot: 'before-desktop', ...(await layoutCheck(`${BASE}/before.html`, { width: 1440, height: 900 })) })
  await page.screenshot({ path: `${OUT}/sidebar-before-desktop.png` })

  // After, short viewport (nav taller than viewport)
  ...
  const scrolled = await page.evaluate(() => {
    ...
    const hb = header.getBoundingClientRect()
    return {
      headerWithinViewportAfterScroll:
        hb.top >= 0 && hb.bottom <= window.innerHeight,
      footerWithinViewportAfterScroll:
        fb.top >= 0 && fb.bottom <= window.innerHeight,
    }
  })
  results.push({ shot: 'after-short-viewport-scrolled', ...scrolled })

  // Before, short viewport (footer below fold, no in-sidebar scroll)
  results.push({ shot: 'before-short-viewport', ...(await layoutCheck(`${BASE}/before.html`, { width: 1280, height: 520 })) })
  ...
  results.push({
    ...
    drawerVisible: await page.locator('#mobileSidebar').isVisible(),
  })
